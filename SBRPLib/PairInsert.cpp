...
void PairInsert::search(class SBRP *S, int u,int best_m,SBRPMove* moves)
{
	...
	S->create_search_neighborhood(u, SBRPLib_SPI_MOVE);    
...
bool PairInsert::evaluate(class SBRP *S, int u, int i, int r, int k, SBRPMove *M)
{
	//u == i ,return fasle
	if(u == i)
		return false;
	...
	S->update_route_info_LNS(i_route);
...
//指针类型的排序，再检查
void PairInsert::insert_SBRPMove(SBRPMove* M,SBRPMove* moves,int best_m)
{
	int rules = SBRPLib_LENGTH_ROUTENUM_SAVING;
...
bool PairInsert::move(SBRP *S, int u, int i, int r, int k, SBRPMove *M)
{
	int d = S->nodes[u]->destination_id;
...
bool PairInsert::remove(SBRP *S,int u)
{
	int d = S->nodes[u]->destination_id;
	
	int r = S->nodes[u]->visited_record_link->first->routenum;
	double u_length = S->route[r].single_length;
	int u_time = S->route[r].single_time;
	//S->Show_one_route(r);
	S->RemoveNode(u,r);	
	S->RemoveNode(d,r);
	S->update_route_info_LNS(r);
	...
	S->total_route_length += S->route[r].single_length - u_length;
	S->total_service_time += S->route[r].single_time - u_time;
	if(S->route[r].num_customers == 0)
	{
		S->total_number_of_routes--;
	}
	return true;
}